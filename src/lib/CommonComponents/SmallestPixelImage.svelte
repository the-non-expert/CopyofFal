<script>
  ...
  const canvasWidth = 5; // Set the canvas width to match the display size
  const canvasHeight = 5; // Set the canvas height to match the display size
  ...
  const colorDistribution = {
    "#99EDFF": Math.floor(totalPixels * 0.6),
    "#A15DF9": Math.floor(totalPixels * 0.3),
    "#F7EC37": Math.floor(totalPixels * 0.1),
  };
...
<div class="pointer-events-none absolute">
  <canvas
    ...
    class="w-[10px] h-[10px] lg:w-[10px] lg:h-[10px]"
  ></canvas>
</div>
